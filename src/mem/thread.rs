...
impl Frame {
    ...
    pub fn new(max_local_variables: usize, max_operand_stack: usize) -> Frame {
        let mut local_variables: Vec<Type> = Vec::with_capacity(max_local_variables);
        // local_variables.resize(max_local_variables, Type::Ref(None));

        for _i in 0..max_local_variables {
            local_variables.push(Type::Ref(None));
        }

        let operand_stack: Vec<Type> = Vec::with_capacity(max_operand_stack);
    ...
    pub fn local_variables_set(&mut self, index: usize, var: Type) {
    ...
    pub fn local_variables_set_last(&mut self, var: Type) {
    ...
    /// Clone
    pub fn local_variables_get(&mut self, index: usize) -> Type {
    ...
    pub fn operand_stack_push(&mut self, var: Type) {
    ...
    pub fn operand_stack_pop(&mut self) -> Type {
    ...
    pub fn operand_stack_extend(&mut self, tail: &mut Vec<Type>) {
        tail.reverse();
